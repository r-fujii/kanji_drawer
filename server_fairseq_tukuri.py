...
tukuri_rule = kanji_parser.load_rule('./kanji/rule/tukuri_rule.json')
...
def post():
    ...
    shape_cts = [kanji_parser.judge_avl_shapes(tukuri_rule, seq) for seq in seqs]
    ...
    return jsonify({"paths": {rank: {"shape": shape_ct, "char": char, "score": score, "attention": att, "neighbors": sim, "path": path} for rank, (shape_ct, char, score, att, sim, path) in enumerate(zip(shape_cts, seqs, scores, att_weights, sims_trainexs_tukuri, paths), start=1)}})
